...
def augment_text(text):
    ...
    augmented_texts = [
        ...
        re.sub(r'\s+', ' ', re.sub(r'[^\w\s]', '', text))
    ]
    return augmented_texts

def preprocess_texts(df):
    """Preprocess texts using TF-IDF vectorization with augmentation"""
    ...
    for idx, text in enumerate(df['Text']):
        ...
        # Repeat the hate column values for each augmented text
        row_hate_values = df.loc[idx, hate_columns].tolist()
        augmented_hate_data.extend([row_hate_values] * len(augmented_texts))
    
    augmented_df = pd.DataFrame({'Text': augmented_data})
    augmented_df[hate_columns] = augmented_hate_data
    
    # Combine hate columns and convert to binary
    augmented_df['hate_label'] = (augmented_df[hate_columns].sum(axis=1) > 0).astype(int)
    
    # Vectorize text
    vectorizer = TfidfVectorizer(max_features=1000, 
                                 stop_words='english', 
                                 min_df=2)
    X = vectorizer.fit_transform(augmented_df['Text']).toarray()
    
    return X, vectorizer, augmented_df
...
def train_lstm(df):
    ...
    try:
        ...
        X, vectorizer, augmented_df = preprocess_texts(df)
        y = augmented_df['hate_label'].values
        ...
        # Create datasets and weighted sampler
        ...
        # Loss and optimizer
        criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([class_counts[0] / class_counts[1]]))
...
def create_streamlit_app():
    ...
    try:
        if os.path.exists('models/lstm_model.pth'):
            ...
            # Recreate the model with the current architecture
            model = ImprovedLSTMModel(
                ...
                output_size=1,
                num_layers=2,         # Revert to 2 layers
            )
